...
fn main_loop(event_loop: EventLoop<()>, instance: Arc<Instance>) {
    ...
    let mut vulkan_connector =
        create_vulkan_connector(device.clone(), queue.clone(), swapchain.clone(), &images);
        ...
        match event {
            Event::WindowEvent {
                ...
                if state == ElementState::Pressed && button == MouseButton::Left {
                    vulkan_connector = create_vulkan_connector(
                        device.clone(),
                        queue.clone(),
                        swapchain.clone(),
                        &images,
...
fn main() {
    ...
    main_loop(event_loop, instance.clone());
}
